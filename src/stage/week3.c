...
static const uint8_t win_cols[][3] = {
    { 49, 162, 253},
    { 49, 253, 140},
    {251,  51, 245},
    {253,  69,  49},
...
{
    const uint8_t *win_col = win_cols[RandomRange(0, COUNT_OF(win_cols) - 1)];
...
void Back_Week3_DrawBG(StageBack *back)
{
    ...
    if (this->win_time >= 0)
    {
        ...
        uint8_t win_r = (((fixed_t)this->win_r * this->win_time) >> FIXED_SHIFT) / 6;
        uint8_t win_g = (((fixed_t)this->win_g * this->win_time) >> FIXED_SHIFT) / 6;
        uint8_t win_b = (((fixed_t)this->win_b * this->win_time) >> FIXED_SHIFT) / 6;
...
StageBack *Back_Week3_New(void)
{
    ...
    Back_Week3 *this = (Back_Week3*)malloc(sizeof(Back_Week3));
    if (this == NULL)
        return NULL;
